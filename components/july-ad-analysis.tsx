...
import {
  julyAdContribution,
  julyAdInsights,
  julyAdScreenshotTrends,
  julyAdStoreColors,
  julyAdStoreOrder,
  julyAdStores,
  type JulyAdStoreKey,
  type JulyAdTrendMetric,
} from "@/lib/ad-analysis-july";

type JulyAdShopKey = (typeof julyAdStoreOrder)[number];
...
const shopLabels: Record<JulyAdShopKey, string> = {
  "88": "本土88",
  "70": "本土70",
  "329": "跨境329",
};
...
const contributionRows = [
  { label: "广告订单", values: julyAdContribution.orders },
  { label: "广告费", values: julyAdContribution.spend },
  { label: "广告转化销售额", values: julyAdContribution.adSales },
] as const;

const trendDefinitions: { title: string; metric: JulyAdTrendMetric }[] = [
...
function makeTrendOption(metric: JulyAdTrendMetric, selectedStore: JulyAdStoreKey): EChartsOption {
  const visibleStores: JulyAdShopKey[] = selectedStore === "all" ? [...julyAdStoreOrder] : [selectedStore];
  const pointCount = julyAdScreenshotTrends[visibleStores[0]][metric].length;

  return {
    animationDuration: 650,
    color: visibleStores.map((store) => julyAdStoreColors[store]),
    tooltip: { show: false },
    legend: {
      show: visibleStores.length > 1,
      bottom: 0,
      left: "center",
      itemWidth: 13,
      itemHeight: 3,
      textStyle: { color: "#73778b", fontSize: 10, fontWeight: 700 },
    },
    grid: { left: 5, right: 5, top: 7, bottom: visibleStores.length > 1 ? 24 : 5 },
    xAxis: { type: "category", show: false, boundaryGap: false, data: Array.from({ length: pointCount }, (_, index) => index + 1) },
    yAxis: { type: "value", show: false, min: 0 },
    series: visibleStores.map((store) => ({
      name: shopLabels[store],
      type: "line" as const,
      smooth: .35,
      showSymbol: false,
      lineStyle: { width: selectedStore === "all" ? 2 : 3 },
      data: julyAdScreenshotTrends[store][metric],
    })),
  };
}
...
export default function JulyAdAnalysisPage({ selectedStore }: { selectedStore: JulyAdStoreKey }) {
  const data = julyAdStores[selectedStore];
  ...
  const contributionOption = useMemo<EChartsOption>(() => ({
    ...
    color: julyAdStoreOrder.map((store) => julyAdStoreColors[store]),
    ...
    series: julyAdStoreOrder.map((store) => ({
      name: shopLabels[store],
      type: "bar" as const,
      stack: "contribution",
      barWidth: 21,
      itemStyle: { opacity: selectedStore === "all" || selectedStore === store ? 1 : .38, borderRadius: store === "88" ? [7, 0, 0, 7] : store === "329" ? [0, 7, 7, 0] : 0 },
  ...
  const roasOption = useMemo<EChartsOption>(() => ({
    ...
    yAxis: { type: "category", inverse: true, data: julyAdStoreOrder.map((store) => shopLabels[store]), axisTick: { show: false }, axisLine: { show: false }, axisLabel: { color: "#44495f", fontSize: 11, fontWeight: 700 } },
    series: [{
      type: "bar",
      barWidth: 21,
      label: { show: true, position: "right", color: "#353a50", fontSize: 11, formatter: (params: { value: unknown }) => `${Number(params.value).toFixed(2)}%` },
      data: julyAdStoreOrder.map((store) => ({
        value: julyAdStores[store].summary.roas * 100,
        itemStyle: { color: julyAdStoreColors[store], opacity: selectedStore === "all" || selectedStore === store ? 1 : .34, borderRadius: 8 },
      })),
    }],
  }), [selectedStore]);
  ...
  const salesTrendOption = useMemo<EChartsOption>(() => ({
    ...
    series: [
      { name: "总销售额", type: "line", smooth: .25, symbolSize: 6, lineStyle: { width: 2.5 }, data: periods.map((row) => row.totalSales) },
      { name: "广告转化销售额", type: "line", smooth: .25, symbolSize: 6, lineStyle: { width: 2.5 }, data: periods.map((row) => row.adSales) },
      { name: "广告费", type: "line", smooth: .25, symbolSize: 6, lineStyle: { width: 2.5 }, data: periods.map((row) => row.adSpend) },
    ],
  }), [periods]);
  ...
  return (
    <div className="content-canvas july-ad-canvas">
        ...
        {trendDefinitions.map(({ title }, index) => (
          <article className="panel july-ad-mini-card" key={title}>
            <div className="july-ad-mini-heading">
              <div><h3>{title}</h3><p>截图趋势，仅展示波动</p></div>
            </div>
      ...
      <section className="july-ad-chart-grid july-ad-weekly-grid">
        <article className="panel july-ad-chart-card">
          <div className="panel-heading july-ad-panel-heading"><div><h2>销售与广告投入趋势</h2><p>{data.label} · 总销售额 / 广告转化销售额 / 广告费</p></div><span className="data-source">五周精确数据</span></div>
          <EChart option={salesTrendOption} height={224} />
        </article>
        <article className="panel july-ad-chart-card">
          <div className="panel-heading july-ad-panel-heading"><div><h2>周度业绩对比</h2><p>{data.label} · 7月五个时间段</p></div><span className="data-source">后台周表</span></div>
          <EChart option={weeklyBarOption} height={224} />
        </article>
      </section>

      <section className="panel july-ad-table-panel">
        <div className="panel-heading july-ad-panel-heading"><div><h2>时间段明细表</h2><p>{data.label} · 月度合计与五周明细完全对齐</p></div><span className="data-source">韩元原始口径</span></div>
      ...
      <section className="panel july-ad-insight-panel">
        ...
        <div className="july-ad-insight-grid">
          {julyAdInsights.map((insight, index) => {
